const expectRevert = async (promise) => {
  try {
    ...
    const nonPayableConstructor = error.message.search(' non-payable constructor') > -1;

    assert(invalidOpcode || revert || outOfGas || nonPayableConstructor, `Expected revert, got ${error} instead`);
